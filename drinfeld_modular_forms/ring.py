...
.. RUBRIC:: Background material

We let `K := \mathbb{F}_q(T)` and `K_{\infty} := \mathbb{F}_q((1/T))`.
We define `\mathbb{C}_{\infty}` to be the completion of an algebraic
closure of `K_{\infty}`. The Drinfeld period domain of rank `r > 1` is
defined to be
...
`\Omega^r(\mathbb{C}_{\infty})`, we consider the following `A`-lattice
of rank `r`:

.. MATH::

    \Lambda_w := Aw_1 \oplus \cdots \oplus Aw_{r-1} \oplus Aw_r.

By analytic uniformization, there exists a corresponding Drinfeld module

.. MATH::

    \phi_w : T \mapsto T + g_0(w)\tau + \cdots
    + g_{r-2}(w)\tau^{r-1} + g_{r-1}(w)\tau^{r}

where `g_{r-1}(w)` never vanishes. This Drinfeld module is the universal
...
*Drinfeld modular forms of weight* `q^i - 1`.

One may show that the ring of all Drinfeld modular forms for
`\mathrm{GL}_r(A)` is generated by the forms `g_i`:

.. MATH::

    M^r(\mathrm{GL}_r(A))
    = \mathbb{C}_{\infty}[g_0, \ldots, g_{r-2}, g_{r-1}].
